<script lang="ts">
  ...
  interface Props { id: string; title: string; badge?: string; folded?: boolean; onfold?: (id: string) => void; onclose?: (id: string) => void; onfocus?: (id: string) => void; ondragstart?: (e: MouseEvent) => void; children: Snippet; }
  let { id, title, badge='', folded=false, onfold, onclose, onfocus, ondragstart, children }: Props = $props();
</script>
...
<div class="flex flex-col h-full bg-surface rounded-lg border border-border overflow-hidden" onmousedown={()=>onfocus?.(id)}>
  ...
  <!-- svelte-ignore a11y_no_static_element_interactions -->
  <div class="flex items-center gap-2 px-3 py-1.5 bg-surface2/50 border-b border-border cursor-move select-none shrink-0" onclick={()=>onfold?.(id)} onmousedown={(e)=>ondragstart?.(e)}>
