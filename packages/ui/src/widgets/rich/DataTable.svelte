...
<div class="bg-[#13131a] border border-white/[0.07] rounded-lg p-4 font-sans">
  ...
  {#if columns.length===0&&rows.length===0}
    <p class="text-zinc-600 text-sm">{spec.emptyMessage??'Aucune donnée'}</p>
  {:else}
    <div class="overflow-auto max-h-[480px] rounded border border-white/[0.07]">
      <table class="w-full border-collapse">
        <thead>
          <tr>
            {#each columns as col}
              <th class="sticky top-0 bg-[#1a1a24] border-b-2 border-white/[0.1] px-3 py-2 text-left text-xs font-mono font-medium text-zinc-500 uppercase tracking-wider whitespace-nowrap cursor-pointer select-none hover:text-zinc-300 transition-colors {col.align==='center'?'text-center':col.align==='right'?'text-right':''}"
                onclick={()=>{ if(sortCol===col.key){sortAsc=!sortAsc;}else{sortCol=col.key;sortAsc=true;} }}>
                {col.label}{#if sortCol===col.key}<span class="ml-1 text-[#7c6dfa]">{sortAsc?'↑':'↓'}</span>{/if}
              </th>
            {/each}
          </tr>
        </thead>
        <tbody>
          {#each displayed as row, i}
            <tr class="hover:bg-white/[0.03] transition-colors {spec.striped!==false&&i%2===1?'bg-white/[0.02]':''} {onrowclick?'cursor-pointer':''}"
              onclick={()=>onrowclick?.(row)}>
              {#each columns as col}
                {@const val=row[col.key]}
                <td class="border-b border-white/[0.05] text-zinc-300 overflow-hidden text-ellipsis whitespace-nowrap max-w-[280px] {compact?'px-2 py-1 text-xs':'px-3 py-2 text-sm'} {col.align==='center'?'text-center':col.align==='right'?'text-right':''}">
                  {#if val==null}<span class="text-zinc-600">—</span>
                  {:else if col.type==='boolean'}<span>{val?'✓':'✗'}</span>
                  {:else if col.type==='link'&&typeof val==='string'}<a href={val} class="text-[#7c6dfa] hover:underline">{val}</a>
                  {:else if typeof val==='object'}<code class="text-xs bg-white/[0.05] px-1 py-0.5 rounded">{JSON.stringify(val)}</code>
          ...
          {/each}
          {#if overflow>0}<tr><td colspan={columns.length} class="text-center text-zinc-600 text-xs py-2 px-3">… {overflow} more rows</td></tr>{/if}
        </tbody>
      </table>
    </div>
    <div class="mt-2 text-zinc-600 text-xs">{rows.length} ligne{rows.length!==1?'s':''}</div>
  {/if}
</div>
