...
export async function render(
  ...
  if (!a || !b) return renderEmpty(container, 'turf-difference', 'Pass two polygons as <code>a</code> and <code>b</code>.');

  const { loadTurf } = await import('./shared.js');
  const turf = await loadTurf();
  const fa = asFeature(turf, a);
  const fb = asFeature(turf, b);
  if (!fa || !fb) return renderEmpty(container, 'turf-difference', 'Inputs must be polygons.');

  let result: any = null;
  try {
    result = turf.difference(turf.featureCollection([fa, fb]));
  } catch (e) {
