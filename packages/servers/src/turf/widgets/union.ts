...
export async function render(
  ...
  if (!a || !b) return renderEmpty(container, 'turf-union', 'Pass two polygons as <code>a</code> and <code>b</code>.');

  const turfMod = await import('@turf/turf');
  const turf = turfMod.default ?? turfMod;
  const fa = asFeature(turf, a);
  const fb = asFeature(turf, b);
  if (!fa || !fb) return renderEmpty(container, 'turf-union', 'Inputs must be polygons.');

  let result: any = null;
  try {
    result = turf.union(turf.featureCollection([fa, fb]));
  } catch (e) {
