...
/**
 * Lazy-load only the Turf submodules actually used by widgets, in parallel.
 * Replaces the monolithic `@turf/turf` import (~528 KB) with atomic imports
 * (~50-80 KB total). The returned object exposes the same `turf.X(...)` API
 * the widgets already rely on, so no widget code needs to change.
 */
export async function loadTurf(): Promise<any> {
  ...
  const [
    ...
    import('@turf/bearing'),
    import('@turf/bezier-spline'),
    import('@turf/boolean-contains'),
    import('@turf/boolean-intersects'),
    import('@turf/boolean-overlap'),
    import('@turf/buffer'),
    import('@turf/center'),
    import('@turf/center-of-mass'),
    import('@turf/centroid'),
    import('@turf/clean-coords'),
    import('@turf/clusters-dbscan'),
    import('@turf/clusters-kmeans'),
    import('@turf/collect'),
    import('@turf/concave'),
    import('@turf/convex'),
    import('@turf/destination'),
    import('@turf/difference'),
    import('@turf/distance'),
    import('@turf/envelope'),
    import('@turf/flip'),
    import('@turf/helpers'),
    import('@turf/hex-grid'),
    import('@turf/intersect'),
    import('@turf/midpoint'),
    import('@turf/nearest-point'),
    import('@turf/point-grid'),
  ...
  _turf = {
    ...
    area: def(area),
    bbox: def(bbox),
    bboxPolygon: def(bboxPolygon),
    bearing: def(bearing),
    bezierSpline: def(bezierSpline),
    booleanContains: def(booleanContains),
    booleanIntersects: def(booleanIntersects),
    booleanOverlap: def(booleanOverlap),
    buffer: def(buffer),
    center: def(center),
    centerOfMass: def(centerOfMass),
    centroid: def(centroid),
    cleanCoords: def(cleanCoords),
    clustersDbscan: def(clustersDbscan),
    clustersKmeans: def(clustersKmeans),
    collect: def(collect),
    concave: def(concave),
    convex: def(convex),
    destination: def(destination),
    difference: def(difference),
    distance: def(distance),
    envelope: def(envelope),
    flip: def(flip),
    // helpers re-exports several factory funcs
    ...
    nearestPoint: def(nearestPoint),
    pointGrid: def(pointGrid),
    pointsWithinPolygon: def(pointsWithinPolygon),
    randomPoint: random.randomPoint,
    sample: def(sample),
    simplify: def(simplify),
    squareGrid: def(squareGrid),
    tag: def(tag),
    tin: def(tin),
    triangleGrid: def(triangleGrid),
    union: def(union),
  };
  return _turf;
}
