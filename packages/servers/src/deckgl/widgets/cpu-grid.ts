...
export async function render(container: HTMLElement, data: Record<string, unknown>): Promise<void | (() => void)> {
  ...
  if (!points.length) return renderEmpty(container, 'deckgl-cpu-grid', 'Provide <code>{points: [{lng, lat, weight?}], cellSize?: meters}</code>.');

  const { center = [points[0].lng, points[0].lat], zoom = 4, style, pitch = 40, cellSize = 1000, extruded = true, elevationScale = 20 } = data as any;

  const { CPUGridLayer } = await import('@deck.gl/aggregation-layers');
