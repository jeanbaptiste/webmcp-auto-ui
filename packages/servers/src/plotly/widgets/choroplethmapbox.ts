...
export async function render(container: HTMLElement, data: Record<string, unknown>): Promise<void | (() => void)> {
  ...
  const traces = [{ type: 'choroplethmapbox', geojson, locations, z, colorscale, featureidkey }];
  const layout = { ...darkLayout(title), mapbox: { style: 'carto-darkmatter', zoom, center: center || { lat: 0, lon: 0 } } };
