...
export async function render(container: HTMLElement, data: Record<string, unknown>): Promise<void | (() => void)> {
  ...
  const cy = cytoscape({
    ...
    style: data.style as any[] || [
      { selector: 'node', style: { 'background-color': '#0ea5e9', 'label': 'data(label)', 'color': '#fff', 'text-valign': 'center', 'font-size': '10px', 'shape': 'round-rectangle', 'width': 'label', 'padding': '10px' } },
