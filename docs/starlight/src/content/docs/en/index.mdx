...
WebMCP Auto-UI is a platform for automatic user interface generation driven by AI agents. It brings together the **Model Context Protocol (MCP)** for data integration, an **LLM agent engine** for orchestration, and a **declarative widget system ([WebMCP](/webmcp-auto-ui/en/concepts/webmcp/))** for client-side rendering.
