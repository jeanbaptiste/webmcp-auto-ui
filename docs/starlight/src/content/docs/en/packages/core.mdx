...
Cleans a JSON Schema for LLM API compatibility. Some providers (notably Anthropic) enforce strict constraints on schemas (no `$ref` without `$defs`, no `additionalProperties` in certain contexts, limited depth).
...
:::caution[LLM-compatible schemas]
Some LLM providers impose constraints on JSON Schemas (no `$ref`, no `oneOf`/`anyOf`/`allOf`). Use `sanitizeSchema()` before sending a schema to the LLM, or enable the `sanitize: true` option in the agent loop's `SchemaTransformOptions` (enabled by default).
:::
