...
Node.js apps are deployed to the **root** of their folder (`/opt/webmcp-demos/flex/index.js`), because systemd runs `node index.js` from that directory. If you deploy to the wrong location, the service starts but serves stale code.
:::
