...
Vous allez voir, en 10 minutes vous aurez une application fonctionnelle qui connecte un serveur MCP, interroge Claude, et affiche des widgets generes automatiquement par l'IA. C'est le point de depart ideal pour tout projet webmcp-auto-ui.
...
- **Node.js 18+** installe
- Une **cle API Anthropic** (Claude) -- gratuite pour les premiers essais
...
- Un chat connecte a Claude (Haiku, Sonnet ou Opus au choix)
...
graph LR
    ...
    Chat -->|prompt| Claude[Claude API]
    Claude -->|tool_call| MCP[Serveur MCP distant]
    MCP -->|donnees| Claude
    Claude -->|widget_display| Canvas[Canvas widgets]
    Canvas -->|rendu| U
```
...
Le navigateur ne peut pas appeler l'API Anthropic directement (les cles API ne doivent jamais etre exposees cote client). Le boilerplate inclut un proxy serveur SvelteKit qui relaie les requetes.
...
- `anthropicProxy` reformate le body et l'envoie a l'API Claude
...
Le provider LLM est l'abstraction qui permet de communiquer avec differents modeles de langage. Le boilerplate utilise `RemoteLLMProvider` pour Claude :
...
- **`LLMSelector`** expose un dropdown pour choisir le modele Claude
