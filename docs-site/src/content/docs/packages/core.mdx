...
Nettoie un JSON Schema pour le rendre compatible avec l'API Anthropic. L'API Claude impose des contraintes strictes sur les schemas (pas de `$ref` sans `$defs`, pas de `additionalProperties` dans certains contextes, profondeur limitee).
...
:::caution[Schemas compatibles Anthropic]
L'API Claude impose des contraintes sur les JSON Schemas (pas de `$ref`, pas de `oneOf`/`anyOf`/`allOf`). Utilisez `sanitizeSchema()` avant d'envoyer un schema au LLM, ou activez l'option `sanitize: true` dans les `SchemaTransformOptions` de la boucle agent (active par defaut).
:::
