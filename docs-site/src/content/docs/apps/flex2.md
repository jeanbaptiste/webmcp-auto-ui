...
- **Claude** (haiku, sonnet, opus) via un proxy server-side qui transmet les requetes a l'API Anthropic
...
|----------|-------------|--------|
| `ANTHROPIC_API_KEY` | Cle API Anthropic (server-side `.env`) | requis |
...
| `cacheEnabled` | Cache prompt Anthropic | `true` |
...
Un endpoint SvelteKit qui utilise `anthropicProxy` du package agent pour relayer les requetes vers l'API Anthropic. La cle API est lue depuis l'environnement server-side ou depuis le body de la requete (pour le mode BYOK).
...
| Chemin sur le serveur | `/opt/webmcp-demos/flex2/` (racine) |
|----------------------|--------------------------------------|
| Service systemd | `webmcp-flex2` |
