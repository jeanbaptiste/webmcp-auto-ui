...
Une cle Anthropic (`ANTHROPIC_API_KEY`) n'est necessaire que pour utiliser Claude. L'agent Gemma 4 tourne entierement dans le navigateur, sans cle ni serveur distant.
:::
...
Le depot fait environ 50 Mo (sans les `node_modules`). Il contient quatre packages et huit apps de demo.
...
Creez un fichier `.env.local` a la racine de l'app que vous voulez lancer (par exemple `apps/flex2/.env.local`) :

```bash
# API Claude (optionnel, requis uniquement pour les agents distants)
...
| flex2 | 5174 | App principale (SvelteKit) |
| viewer2 | 5175 | Visionneur HyperSkills |
| showcase2 | 5176 | Galerie de widgets |
| todo2 | 5177 | Demo todo |

Pour lancer une seule app :

```bash
npm run dev:flex2    # Juste flex2
...
Ouvrez `http://localhost:5174` (flex2). Dans le panneau de droite, cliquez sur **Settings** et selectionnez un modele :
...
Si vous choisissez Gemma, le navigateur telecharge les poids du modele (~2-6 Go). Le composant `<GemmaLoader>` affiche la progression en temps reel.
...
│   ├── flex2/         # App SvelteKit principale (composeur)
│   ├── showcase2/     # Galerie de widgets
│   ├── todo2/         # Demo todo
...
// Providers LLM
import { RemoteLLMProvider } from '@webmcp-auto-ui/agent';  // Claude via proxy
...
### Creer un provider Claude

Le `RemoteLLMProvider` se connecte a Claude via un endpoint proxy SvelteKit. La cle API reste cote serveur -- jamais exposee au navigateur.
