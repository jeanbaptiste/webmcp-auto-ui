...
Le deploiement de WebMCP Auto-UI repose sur un **script centralise** (`scripts/deploy.sh`) qui gere les huit apps du monorepo avec leurs chemins et configurations specifiques. Ce guide couvre la procedure complete, du build local jusqu'a la verification en production.
...
graph LR
    ...
    subgraph SERVER["Serveur de production"]
        ...
        NGINX -->|proxy| MULTI[multi-svelte :3012]
...
| **Apps Node.js** | flex2 (:3007), viewer2, recipes, showcase2, boilerplate, multi-svelte |
| **Apps statiques** | home, todo2 (servies par nginx) |
...
|-----|------|-------------------|----------------------|-----------|-------|
| **flex2** | Node.js (SvelteKit) | `apps/flex2/build/` | `/opt/webmcp-demos/flex2/` | `node index.js` | Composeur principal |
| **viewer2** | Node.js (SvelteKit) | `apps/viewer2/build/` | `/opt/webmcp-demos/viewer2/` | `node index.js` | Visionneur HyperSkills |
| **showcase2** | Node.js (SvelteKit) | `apps/showcase2/build/` | `/opt/webmcp-demos/showcase2/` | `node index.js` | Galerie de widgets |
...
| **multi-svelte** | Node.js (SvelteKit) | `apps/multi-svelte/build/` | `/opt/webmcp-demos/multi-svelte/` | `node index.js` | App multi-tab |
| **home** | Static (SvelteKit) | `apps/home/build/` | `/opt/webmcp-demos/home/` | N/A (nginx) | Page d'accueil |
| **todo2** | Static (SvelteKit) | `apps/todo2/build/` | `/opt/webmcp-demos/todo2/` | N/A (nginx) | Demo todo |

:::caution[Chemins differents]
Les apps Node.js sont deployees a la **racine** de leur dossier (`/opt/webmcp-demos/flex2/index.js`), car systemd lance `node index.js` depuis ce dossier. Si vous deployez au mauvais endroit, le service demarre mais sert l'ancien code.
:::
...
# pour les apps statiques (home, todo2)
...
**Symptome** : erreurs 429 dans les logs de flex2.
...
# - .env manquant → ls -la /opt/webmcp-demos/flex2/.env
# - Dependances npm manquantes → cd /opt/webmcp-demos/flex2 && npm install --production
```
...
- [ ] Apres deploiement : verifier les logs (`journalctl -u webmcp-flex2 -n 20`)
