...
graph TD
    ...
    subgraph "Apps"
        FLEX[flex2 — Main composer]
        VIEWER[viewer2 — HyperSkills viewer]
        SHOWCASE[showcase2 — Widget gallery]
        RECIPES[recipes — Recipe explorer]
        HOME[home — Homepage]
        BOILER[boilerplate — Starter template]
        TODO[todo2 — Todo demo]
        MULTI[multi-svelte — Multi-tab]
    end
...
| Claude provider | `RemoteLLMProvider` | `agent` |
...
1. **Is the local build up to date?**
   ```bash
   ls -la apps/flex2/build/index.js   # Check modification date
   ```
...
Existing diagrams are pre-rendered as SVGs in `docs-site/public/diagrams/`. To regenerate SVGs after modifying a diagram:

```bash
npx @mermaid-js/mermaid-cli -i file.mmd -o docs-site/public/diagrams/file.svg --backgroundColor transparent
```
...
```bash
npm run dev:flex2    # Develop on the main app
