...
An Anthropic key (`ANTHROPIC_API_KEY`) is only needed for Claude. The Gemma 4 agent runs entirely in the browser -- no key, no remote server.
:::
...
The repository is about 50 MB (excluding `node_modules`). It contains four packages and eight demo apps.
...
Create a `.env.local` file at the root of the app you want to run (for example `apps/flex2/.env.local`):

```bash
# Claude API (optional, only required for remote agents)
...
| flex2 | 5174 | Main app (SvelteKit) |
| viewer2 | 5175 | HyperSkills viewer |
| showcase2 | 5176 | Widget gallery |
| todo2 | 5177 | Todo demo |

To launch a single app:

```bash
npm run dev:flex2    # Just flex2
...
Open `http://localhost:5174` (flex2). In the right panel, click **Settings** and select a model:
...
│   ├── flex2/         # Main SvelteKit app (composer)
│   ├── showcase2/     # Widget gallery
│   ├── todo2/         # Todo demo
...
`RemoteLLMProvider` connects to Claude through a SvelteKit proxy endpoint. Your API key stays server-side -- never exposed to the browser.
