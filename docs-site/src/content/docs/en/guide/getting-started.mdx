...
An API key (`ANTHROPIC_API_KEY`) is only needed for remote LLM providers (e.g. Claude, Gemini, ChatGPT). The Gemma 4 agent runs entirely in the browser -- no key, no remote server.
:::
