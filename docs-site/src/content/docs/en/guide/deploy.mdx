...
WebMCP Auto-UI deployment relies on a **centralized script** (`scripts/deploy.sh`) that manages all eight monorepo apps with their specific paths and configurations. This guide covers the complete procedure, from local build to production verification.
...
| **Node.js apps** | flex2 (:3007), viewer2, recipes, showcase2, boilerplate, multi-svelte |
...
| **multi-svelte** | Node.js (SvelteKit) | `apps/multi-svelte/build/` | `/opt/webmcp-demos/multi-svelte/` | `node index.js` | Multi-tab app |
