...
The `@webmcp-auto-ui/agent` package is the brain of the platform. It orchestrates the LLM agent loop (prompt, tool call, result, prompt), manages providers (remote Claude, in-browser Gemma WASM, local Ollama), structures tools into layers with lazy loading, and provides a built-in WebMCP server (`autoui`) with over 25 native widgets.
...
Provider for Anthropic Claude via an HTTP proxy. The proxy (typically a SvelteKit `+server.ts`) adds the API key and relays requests to the Anthropic API.
...
:::caution[Main thread]
Gemma runs on the main thread. During inference, the UI may momentarily freeze. For long tasks, use the remote Claude provider instead.
:::
...
Some MCP servers expose schemas with `$ref`, `oneOf`, or `anyOf` that the Anthropic API doesn't support. Sanitization is enabled by default (`sanitize: true`), but check with `runDiagnostics` if you encounter schema errors.
:::
...
Remote sends requests to the Claude API via an HTTP proxy. Wasm downloads and runs the Gemma model directly in the browser with no server. Remote is more powerful but paid; Wasm is free but slower and less capable.
...
Yes, but Ollama models are generally less effective than Claude for tool calls. Test with `runDiagnostics` to verify schema compatibility.
