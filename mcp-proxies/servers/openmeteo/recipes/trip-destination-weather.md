...
const { latitude, longitude, timezone, name, country } = geo.results[0];
...
const d = w.daily;
const tMean = d.temperature_2m_max.reduce((s, v, i) => s + (v + d.temperature_2m_min[i]) / 2, 0) / d.time.length;
const rainyDays = d.precipitation_sum.filter(p => p > 1).length;
const peakUV = Math.max(...d.uv_index_max);
const tMaxAbs = Math.max(...d.temperature_2m_max);
const tMinAbs = Math.min(...d.temperature_2m_min);
...
if (peakUV >= 6) suitcase.push('Lunettes de soleil, creme solaire (UV ' + peakUV.toFixed(0) + ')');
if (tMaxAbs - tMinAbs > 15) suitcase.push('Vetements pour ecarts T jour/nuit');
...
await widget('stat-card', {
  items: [
    { label: 'T moyenne', value: `${tMean.toFixed(1)}C`, icon: 'thermometer' },
    { label: 'Jours pluvieux', value: `${rainyDays}/15`, icon: 'cloud-rain' },
    { label: 'UV max', value: `${peakUV.toFixed(0)}`, icon: 'sun' },
    { label: 'Amplitude', value: `${tMinAbs.toFixed(0)} -> ${tMaxAbs.toFixed(0)}C`, icon: 'activity' }
  ]
});

await widget('kv', {
  title: 'Conseils valise',
  pairs: suitcase.map((s, i) => [`${i + 1}`, s])
});
