...
const sbdb = await call('jpl_sbdb', { sstr: target, cad: true });
...
let sentry = null;
try { sentry = await call('jpl_sentry', { des }); } catch { /* not in Sentry */ }
...
await widget('chart', {
  type: 'scatter',
  data: [
    { x: +get(orb, 'a'), y: +get(orb, 'e'), label: target, color: '#dc2626' },
...
const rows = (cad.data || []).slice(0, 20).map(d => [d[3], d[4], d[7], d[10]]);
await widget('table', {
...
if (sentry?.data?.length) {
  const top = sentry.data[0];
  await widget('text', {
    title: 'Sentry impact risk',
    body: `Cumulative impact probability: ${top.ip}. Palermo Scale: ${top.ps_cum}. Torino Scale: ${top.ts_max ?? 0}. ${top.n_imp} virtual impactors monitored.`
  });
} else {
...
```js
const sbdb = await call('jpl_sbdb', { sstr: 'Bennu', cad: true });
const cad  = await call('jpl_cad',  { des: '101955', date_max: '2200-01-01', dist_max: '0.2' });
const sentry = await call('jpl_sentry', { des: '101955' });
await widget('profile', { name: 'Bennu (101955)', badges: ['NEO', 'PHA', 'Sentry-monitored'] });
await widget('table', { columns: ['Date', 'Dist (au)'], rows: cad.data.slice(0, 10).map(r => [r[3], r[4]]) });
```

### Eros (no impact risk)
```js
const sbdb = await call('jpl_sbdb', { sstr: '433' });
await widget('profile', { name: '433 Eros' });
await widget('kv', { items: [{ label: 'a (au)', value: sbdb.orbit.elements.find(e => e.name === 'a').value }] });
