...
const biggest = rows.reduce((b, r) => +r['impact-e'] > +(b?.['impact-e'] || 0) ? r : b, null);
...
await widget('stat-card', { label: 'Biggest (kt)', value: (+biggest?.['impact-e'] || 0).toFixed(2), icon: 'maximize' });
...
const top = [...rows].sort((a, b) => +b['impact-e'] - +a['impact-e']).slice(0, 15);
...
const d = await call('jpl_fireball', { date_min: start, limit: 200 });
const rows = (d.data || []).map(r => Object.fromEntries(d.fields.map((f, i) => [f, r[i]])));
...
```js
const d = await call('jpl_fireball', { date_min: '2013-01-01', date_max: '2013-12-31' });
await widget('table', { columns: ['Date', 'kt', 'Alt'], rows: d.data.map(r => [r[0], r[8], r[2]]) });
```
