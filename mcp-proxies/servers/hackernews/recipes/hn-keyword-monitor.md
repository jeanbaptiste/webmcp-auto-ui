...
6. **Top 5 posts in cards**:
   ```js
   await widget('cards', {
     items: [...posts].sort((a, b) => b.points - a.points).slice(0, 5).map(p => ({
...
await widget('cards', {
  items: hits.slice(0, 5).map(p => ({ title: p.title, subtitle: `${p.points} pts`, url: p.url }))
});
```
...
await widget('table', {
  ...
  rows: hits.map(p => [p.title, p.points, p.num_comments, p.author])
});
```
