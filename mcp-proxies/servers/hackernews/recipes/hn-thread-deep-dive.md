...
3. **Render KPI stat-cards**:
   ...
   await widget('stat-card', { label: 'Max depth', value: Math.max(0, ...allComments.map(c => c.depth)), icon: 'git-branch' });
...
6. **Comment distribution by depth**:
   ```js
   const byDepth = {};
   for (const c of allComments) byDepth[c.depth] = (byDepth[c.depth] || 0) + 1;
...
function flatten(n, d = 0, acc = []) {
  for (const c of (n.children || [])) { acc.push({ ...c, depth: d }); flatten(c, d + 1, acc); }
  return acc;
}
const comments = flatten(item);

await widget('stat-card', { label: 'Score', value: item.points, icon: 'arrow-up' });
await widget('stat-card', { label: 'Comments', value: comments.length, icon: 'message-circle' });
await widget('text', { title: item.title, body: item.text || item.url, html: !!item.text });
await widget('cards', {
...
await widget('text', { title: poll.title, body: poll.text || '', html: true });
await widget('chart-rich', {
  type: 'bar',
  title: 'Poll options',
  data: (poll.children || []).map(opt => ({ label: opt.text || '?', value: opt.points || 0 }))
});
```
