...
1. **List all sections** to confirm the target exists:
   ```js
   const all = await call('get_sections', { title: 'Internet' });
   ```
...
2. **Render the table of contents** (so the user sees siblings):
   ...
   await widget('table', {
     ...
     rows: all.sections.map(s => [s.title, s.level])
   });
   ```
...
3. **Summarize the targeted section**:
   ...
   await widget('text', { content: sec.summary });
   ```
...
```js
const all = await call('get_sections', { title: 'Internet' });
const sec = await call('summarize_article_section', { title: 'Internet', section_title: 'History', max_length: 250 });
...
```js
const all = await call('get_sections', { title: 'France' });
const sec = await call('summarize_article_section', { title: 'France', section_title: 'Économie', max_length: 280 });
