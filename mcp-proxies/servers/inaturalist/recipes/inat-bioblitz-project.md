...
await widget('stat-card', { label: 'Contributors', value: board.results.length, icon: 'users' });
...
await widget('gallery', {
  images: obs.results
    ...
    .map(o => ({
      ...
      caption: `${o.species_guess} — ${o.user?.login}`,
    })),
});
...
await widget('stat-card', { label: p.title, value: p.observations_count || '—', icon: 'flag' });
const board = await call('observers_leaderboard', { place_id: p.place_id, per_page: 10 });
await widget('table', { columns: ['Rank', 'User', 'Species'], rows: board.results.map((r, i) => [i + 1, r.user.login, r.species_count]) });
```
