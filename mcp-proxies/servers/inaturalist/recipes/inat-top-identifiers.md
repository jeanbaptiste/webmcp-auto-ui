...
const detail = await call('get_taxon', { id: target.id });

// 3. Top identifiers leaderboard
const ids = await call('top_identifiers', {
  taxon_id: target.id,
...
await widget('kv', {
  ...
  items: {
    ...
    'Total identifiers ranked': ids.results.length,
  },
});

// 5. Profile of the #1 identifier
const champ = ids.results[0].user;
await widget('profile', {
  title: champ.name || champ.login,
  subtitle: '@' + champ.login,
  image: champ.icon_url,
  fields: {
    'Identifications on this taxon': ids.results[0].count,
    'Total iNat IDs': champ.identifications_count,
...
await widget('table', {
  ...
  rows: ids.results.map((r, i) => [
    i + 1,
    r.user.login,
    r.count,
    r.user.identifications_count,
  ]),
});
```
...
const ids = await call('top_identifiers', { taxon_id: t.id, per_page: 10 });
await widget('table', { columns: ['Rank', 'User', 'IDs'], rows: ids.results.map((r, i) => [i + 1, r.user.login, r.count]) });
```

### Top experts for golden eagle
```js
const t = (await call('search_taxa', { q: 'Aquila chrysaetos', per_page: 1 })).results[0];
const ids = await call('top_identifiers', { taxon_id: t.id, per_page: 5, quality_grade: 'research' });
const top = ids.results[0].user;
await widget('profile', { title: top.name || top.login, subtitle: '@' + top.login, image: top.icon_url });
```
